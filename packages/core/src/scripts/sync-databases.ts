 ...
 * 
 * Sincroniza dados dos bancos operacionais para seshat.db (Prisma)
 * - memories.db → seshat.db (memories)
 * - vector-store.db → seshat.db (projects, documents)
 * - search-analytics.db → seshat.db (search_queries, cache_stats)
 */
...
const th0thDbPath = path.join(dataDir, "seshat.db");
