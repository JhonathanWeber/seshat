...
// ── Mock dependencies ────────────────────────────────────────
mock.module("@seshat/shared", () => {
  const actual = require("@seshat/shared");
  return {
    ...actual,
    config: {
      get: (key: string) => {
        if (key === "dataDir") return "/tmp/seshat-test-search-ctrl";
        const defaults: Record<string, any> = {
          vectorStore: { type: "sqlite", dbPath: "/tmp/seshat-test-vs.db", collectionName: "test", embeddingModel: "default" },
          keywordSearch: { dbPath: "/tmp/seshat-test-kw.db", ftsVersion: "fts5" },
          cache: { l1: { maxSize: 1024, defaultTTL: 60 }, l2: { dbPath: "/tmp/seshat-test-cache.db", maxSize: 1024, defaultTTL: 60 }, embedding: { dbPath: "/tmp/seshat-test-emb-cache.db", maxAgeHours: 1 } },
