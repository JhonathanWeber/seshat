...
import { configExists, initConfig, loadConfig } from "@seshat/shared/config"

// ---------------------------------------------------------------------------
// Config
// ---------------------------------------------------------------------------

const SESHAT_API_URL = process.env.SESHAT_API_URL || "http://localhost:3333"
...
function ensureConfig(): void {
  if (!configExists()) {
    ...
    console.log(`
╔═══════════════════════════════════════════════════════════════╗
║  seshat initialized with default configuration                  ║
║                                                                ║
║  Config: ~/.config/seshat/config.json                           ║
║  Provider: Ollama (local, free)                                ║
║                                                                ║
║  To change provider:                                           ║
║    npx seshat-config use mistral --api-key YOUR_KEY             ║
║    npx seshat-config use openai --api-key YOUR_KEY              ║
╚═══════════════════════════════════════════════════════════════╝
`)
  }
}
...
async function th0thFetch<T = unknown>(
  ...
  try {
    const res = await fetch(`${SESHAT_API_URL}${endpoint}`, {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      throw new Error(`seshat ${res.status}: ${text.slice(0, 200)}`)
    }
    return res.json()
  } finally {
    clearTimeout(timer)
...
export const SeshatPlugin: Plugin = async ({ project, directory, worktree, client }: PluginInput) => {
  ...
  function log(level: "debug" | "info" | "warn" | "error", message: string, extra?: Record<string, unknown>) {
    client.app.log({
      body: { service: "seshat", level, message, extra },
    }).catch(() => {})
  }

  function toast(message: string, variant: "success" | "error" | "info" = "info") {
    client.tui.showToast({
      body: { message: `[seshat] ${message}`, variant },
    }).catch(() => {})
  }
  ...
  return {
    tool: {
      ...
      "seshat_remember": tool({
        description:
          "Store important information in seshat memory. Persists across sessions. Use for: user preferences, architectural decisions, discovered patterns.",
        args: {
      ...
      "seshat_index_status": tool({
        description:
          "Check the status and progress of an async indexing job. Use the jobId returned by seshat_index.",
        args: {
          jobId: tool.schema.string().describe("Job ID returned by seshat_index"),
        },
        async execute(args) {
      ...
      "seshat_analytics": tool({
        description: "Get seshat usage analytics and performance metrics.",
        args: {
    ...
    "session.created": async () => {
      try {
        ...
        const res = await fetch(`${SESHAT_API_URL}/health`, { signal: controller.signal })
        clearTimeout(timer)
        apiAvailable = res.ok
        if (apiAvailable) {
          log("info", `Connected to seshat API at ${SESHAT_API_URL}`)
        } else {
          toast("seshat API unhealthy", "error")
        }
      } catch {
        apiAvailable = false
        log("warn", `seshat API unreachable at ${SESHAT_API_URL}`)
      }
    },
      ...
      try {
        ...
        if (memories?.results?.length) {
          const memoryText = memories.results
            ...
            .join("\n")
          output.context.push(`## seshat - Persistent Memories\n${memoryText}`)
        }
      } catch (err) {
    ...
    // Inject seshat env vars into shell
    // Hooks interface: shell.env(input: { cwd, sessionID?, callID? }, output: { env })
    "shell.env": async (_input, output) => {
      output.env.SESHAT_PROJECT_ID = projectId
      output.env.SESHAT_PROJECT_PATH = projectPath
      output.env.SESHAT_API_URL = SESHAT_API_URL
    },
