...
function help() {
  console.log(`
seshat-config - Configuration manager for seshat

Usage:
  seshat-config <command> [options]

Commands:
  init              Initialize seshat configuration
...
Examples:
  seshat-config init
  seshat-config init --mistral your-api-key
  seshat-config use ollama --model bge-m3:latest
  seshat-config use mistral --api-key your-key
  seshat-config set embedding.dimensions 1024
`);
}
...
switch (command) {
  ...
  case "show": {
    if (!configExists()) {
      console.log("No config file found. Run `seshat-config init` to create one.");
      console.log("\nUsing defaults:");
      console.log(JSON.stringify(defaultSeshatConfig, null, 2));
      process.exit(0);
    }
  ...
  case "set": {
    ...
    if (!key || !value) {
      console.error("Usage: seshat-config set <key> <value>");
      process.exit(1);
    }
